...
void Staff::saveToDatabase(bool mode, DatabaseHandler& db_handler, const Staff* object,
                           const std::vector<std::vector<std::string>>& objects) {
    if (mode == SINGLE_REQUEST_SEND && object != nullptr) {
        db_handler.insert("staff", {{"full_name", object->full_name},
...
std::vector<Staff> Staff::getUniqueFromDatabase(DatabaseHandler& db_handlere) {
    ...
    db_handlere.select("Staff", {"full_name", "birth_date", "gender"}, "", "", "full_name", -1, callback,
                       &staff);
    return staff;
}
...
std::vector<Staff> Staff::getFromDatabase(DatabaseHandler& db_handlere) {
    std::vector<Staff> staff;
    db_handlere.select("Staff", {"full_name", "birth_date", "gender"}, "gender = 'Male'",
                       "full_name LIKE 'F%'", "", -1, callback, &staff);
    return staff;
}
...
void Staff::optimizeDatabaseOff(DatabaseHandler& db_handler) {
    db_handler.optimizeDatabaseOff({"full_name", "gender"});
}
